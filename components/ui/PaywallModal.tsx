          ...
          <View style={styles.plans}>
            {plans.map((plan) => (
              <View key={plan.name} style={[styles.planRow, { borderColor: plan.border }]}>
                <LinearGradient colors={[plan.bg, 'transparent']} style={StyleSheet.absoluteFill} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} />
...
function makeStyles(C: ColorPalette) {
  return StyleSheet.create({
    ...
    shimmer: {
      ...
      right: 0,
      height: 1,
      backgroundColor: C.borderGlassLight,
    },
    iconWrap: {
      width: 64,
      height: 64,
    ...
    plans: { width: '100%', gap: 10, marginTop: 4 },
    planRow: {
      flexDirection: 'row',
      alignItems: 'center',
    ...
    planInfo: { gap: 3 },
    planName: { fontSize: 15, fontWeight: '800', letterSpacing: -0.3 },
    planPrice: { fontSize: 12, color: C.textMuted, fontWeight: '500' },
    planArrow: {
      width: 28,
      height: 28,
      borderRadius: 8,
    ...
    comingSoonText: { fontSize: 13, color: PRO_COLOR, fontWeight: '600' },
    closeBtn: {
      width: '100%',
      paddingVertical: 15,
      borderRadius: 14,
    ...
    closeBtnText: { fontSize: 15, fontWeight: '700', color: C.textSub },
  });
}
